...
def main():
    ...
    while(True):
        try:
            ...
            if code.strip() != "/":
                price = float(get_property(code.strip(),"price"))
                subtotal = price*float(product_quantity)
                name = get_property(code.strip(),"name")
                code_details = [code.strip(), name, product_quantity.strip(), subtotal]
                formatting_list[code.strip()] = code_details
                continue
            continue
        except ValueError:
